...
CLASS zrap100_tc_travel_eml_sol DEFINITION
  ...
  PRIVATE SECTION.
    ...
    CLASS-METHODS:
      "! class method to set up test double framework
      class_setup,
      "! class method to stop test doubles
      class_teardown.
   METHODS:
      "! instance method to reset test doubles
      setup,
      "! instance method to roll back any changes
      teardown.

   METHODS:
      "! Code Under Test (CUT): Create with action call and commit
...
*            FROM VALUE #( ( %cid_ref = 'ROOT1' ) )
     ...
     SELECT * FROM ZRAP100_R_TravelTP_SOL INTO TABLE @DATA(lt_travel). "#EC CI_NOWHERE
